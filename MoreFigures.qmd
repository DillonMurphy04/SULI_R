...
label_data_1 <- df_race_deviation %>%
  filter(Changed == "NO CHANGE") %>%
  filter(Race == "American Indian or Alaska Native")
label_data_2 <- df_race_deviation %>%
  filter(Changed == "NO CHANGE") %>%
  filter(Race == "White")

race_graph <- ggplot(df_race_deviation %>% filter(Changed == "NO CHANGE"), 
                         aes(x = deviation_from_mean, y = reorder(Race, deviation_from_mean))) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_x_continuous(labels = scales::percent_format()) +
  labs(title = "Race",
       x = "",
...
gender_graph <- ggplot(df_gender_deviation %>% filter(Changed == "NO CHANGE"), 
                         aes(x = deviation_from_mean, y = reorder(Gender, deviation_from_mean))) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_x_continuous(labels = scales::percent_format()) +
  labs(title = "Gender",
       x = "",
...
race_graph / gender_graph +   
  plot_annotation(
    caption = "Figure 4. Deviation from the mean proportion of perceived institutional support by race and gender for individuals who did not change their career intentions, ordered decreasingly.\nThus top groups saw the largest increase in percieved institutional support (of those who did not express change in career intentions). Data suggest underrepresented individuals\nwho did not express change in career intentions percieved more feelings of institutional support.",
    theme = theme(plot.caption = element_text(hjust = 0, margin = margin(t = 0, r = 0, b = 0, l = 0), size = 16))
  )

...
generate_summary_table <- function(df, column, category_name) {
  ...
  summary_df <- df %>%
    ...
    rename(Category = {{column}}) %>%
    arrange(desc(count)) %>% 
    mutate(Category = ifelse(Category == "Gender fluid/queer/non-conforming", "Genderfluid, Genderqueer, or Gender Non-conforming", Category))
...
combined_table <- paste(
  "<h2 style='text-align: left; font-size: 12pt;'>Table 1: Respondent Demographic Information (N=36)</h2>",
...
concern_counts_professional %>%
  pivot_wider(names_from = Category, values_from = c(pre_covid_prop, post_covid_prop)) %>% 
  kable(col.names = c("Race/Ethnicity", "Pre-COVID Prop.", "New COVID Prop.", "Pre-COVID Prop.", "New COVID Prop.", "Pre-COVID Prop.", "New COVID Prop."),

...
```{r}
word_data <- data.frame(
  Professional = data$Feel.free.to.comment.on.any.of.the.items.you.marked.above..1,
  Personal = data$Feel.free.to.comment.on.any.of.the.items.you.marked.above.,
  Changed_Goals = data$X.Optional..Please.explain.or.give.an.example.,
  Changed_Projects = data$X.Optional..Please.explain.or.give.an.example..1
)

responses <- word_data %>%
  pivot_longer(cols = c(Professional, Personal, Changed_Goals, Changed_Projects), 
               names_to = "Type", 
               values_to = "Response") %>%
  filter(!is.na(Response))
...
ggplot(top_words, aes(x = reorder_within(word, n, Type), y = n, fill = Type)) +
  ...
  facet_wrap(~Type, scales = "free_y") +
  scale_x_reordered() +
  labs(y = "Frequency", x = "Words", title = "Most Common Words in Optional Responses") +
  coord_flip()
